...
% --- Highlight macros for table best/second-best values ---------------------
% xcolor + colortbl are already loaded by the mdpi class (see Definitions/mdpi.cls).
\definecolor{BestColor}{RGB}{200,30,30}   % strong red for the best value
\definecolor{SndColor}{RGB}{30,90,200}    % strong blue for the second-best value
% \best / \snd wrap text-mode cells (mean \pm std written with \(\pm\) toggles).
\newcommand{\best}[1]{\textcolor{BestColor}{\textbf{#1}}}
\newcommand{\snd}[1]{\textcolor{SndColor}{\underline{#1}}}
% \bestm wraps a pure math-mode cell that previously used \boldsymbol{...}.
\newcommand{\bestm}[1]{\textcolor{BestColor}{\boldsymbol{#1}}}
% ---------------------------------------------------------------------------
...
\newcommand{\orcidauthorA}{0009-0002-7028-5577}
\newcommand{\orcidauthorB}{0000-0001-9234-0026}

\Author{Shiyi Xu $^{1}$, Mohan Xu $^{2}$\orcidA{}, and Erjin Zhou $^{3,*}$\orcidB{}}

\AuthorNames{Shiyi Xu, Mohan Xu, and Erjin Zhou}

\address{%
$^{1}$ \quad Capital Medical University, Beijing, China; xsy4831@mail.ccmu.edu.cn\\
$^{2}$ \quad School of Physics, Peking University, Beijing, China; 2200011601@stu.pku.edu.cn\\
$^{3}$ \quad Dexmal, Beijing, China; zhouerjin@gmail.com}

\corres{Correspondence: Erjin Zhou; \href{mailto:zhouerjin@gmail.com}{zhouerjin@gmail.com}}
...
The reported runs use a single NVIDIA RTX PRO~6000 Blackwell Workstation Edition (97{,}887\,MiB VRAM, CUDA~12.8); because Blackwell-class GPUs require a recent CUDA path, training and profiling were executed under PyTorch~2.12 \cite{paszke2019pytorch} (\texttt{cu128}). To make this independent of any development build, the released reproducibility package documents an environment specification (Python~3.12, PyTorch~\(\geq\)2.5, antspyx, and SimpleITK) under which the same training and evaluation scripts run unmodified on prior-generation CUDA-capable hardware. PyTorch~2.12 is therefore a hardware-availability artefact rather than a method dependency. Baselines are evaluated under the same test protocol; classical baselines use ANTs/ANTsPyX and SimpleITK implementations \cite{avants2008ants,antspyx2024,simpleitk2013}.

\subsection{Evaluation Metrics}
The primary numerical contrasts in Table~\ref{tab2} and Table~\ref{tab:oasis_zeroshot} use grouped mean Dice over 17 VOI groups (higher is better), HD95 and ASSD in millimetres (lower is better), the non-positive Jacobian determinant ratio \(\#\{x:\det J_\phi(x)\le 0\}/\#\{x\}\) (lower is better), and SDlogJ, \(\mathrm{std}_x(\log\max(\det J_\phi(x),\epsilon))\) (lower is better)~\cite{hering2023learn2reg}. Figure~\ref{fig4} and Figure~S1 summarize Jacobian and overlap trends in compact form. Supplementary Table~S2 reports auxiliary descriptors for a focused model subset that use the same evaluation code path but are omitted from Table~\ref{tab2} for space: NSD@1\,mm~\cite{nikolov2021nsd}, voxel-averaged bending energy, mean absolute divergence of the displacement field \cite{bookstein1989tps,mok2020lapirn}, and Jacobian tail quantiles \((J_{\min}, J_{p01}, J_{p99}, J_{\max})\). Training uses local squared cross-correlation as \(\mathcal{L}_{\mathrm{sim}}\) (Section~\ref{sec:loss}).

\subsection{Grouped-VOI Label Protocol}
Grouped Dice uses a 46-label FreeSurfer-compatible index set \cite{fischl2002freesurfer}, with labels aggregated into 17 bilateral or anatomically related groups. The explicit group-to-label-ID mapping is listed in Supplementary Table~S5.
...
\midrule
HypEReg-TransMorph & \best{0.7537 \(\pm\) 0.0275} & 0.000015 \(\pm\) 0.000007 & 0.3280 \(\pm\) 0.0221 & \snd{5.3234 \(\pm\) 0.6936} & \best{1.3570 \(\pm\) 0.1770}\\
TransMorph & 0.7527 \(\pm\) 0.0305 & 0.015021 \(\pm\) 0.003416 & 0.5064 \(\pm\) 0.0250 & 5.6872 \(\pm\) 0.7810 & \snd{1.4073 \(\pm\) 0.1819}\\
TransMorphBayes & \snd{0.7530 \(\pm\) 0.0302} & 0.015634 \(\pm\) 0.003363 & 0.4920 \(\pm\) 0.0330 & 5.7246 \(\pm\) 0.7604 & 1.4160 \(\pm\) 0.1832\\
...
MIDIR & 0.7423 \(\pm\) 0.0228 & \best{0.000000 \(\pm\) 0.000000} & \snd{0.3148 \(\pm\) 0.0242} & \best{5.3028 \(\pm\) 0.6139} & 1.4100 \(\pm\) 0.1585\\
...
SyN (ANTs) & 0.6445 \(\pm\) 0.0397 & \snd{0.000001 \(\pm\) 0.000005} & \best{0.2996 \(\pm\) 0.0725} & 6.4457 \(\pm\) 1.8104 & 1.5233 \(\pm\) 0.7748\\
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Values are reported as mean \(\pm\) standard deviation over 115 test subjects. Dice is the grouped mean over 17 anatomical structure groups; \(\det(J_\phi)\le 0\) ratio, SDlogJ, HD95, and ASSD are taken from per-case evaluation exports. \textcolor{BestColor}{\textbf{Bold red}} indicates the best value per metric; \textcolor{SndColor}{\underline{blue underline}} indicates the second-best.}}
\end{table}
...
Because the two terms act on largely disjoint regions of \(\det J_\phi\) space, they provide complementary supervision: \(\mathcal{L}_{\mathrm{volume}}\) shapes the global distribution, whereas \(\mathcal{L}_{\mathrm{fold}}\) supplies a localized anti-folding signal. Table~\ref{tab:ablation} quantifies the individual term contributions and coefficient sensitivity on the 115-subject IXI test set, while Supplementary Table~S2 reports aligned Jacobian-tail evidence (\(J_{\min},J_{p01},J_{p99},J_{\max}\)) for representative term-isolation variants.

\paragraph{Role of each term.} The fold-hinge-only configuration (\(\beta=0,\,\gamma=20\)) maintains competitive Dice (0.7503) but leaves the non-positive Jacobian ratio at \(4.69\times10^{-3}\), comparable to unregularized Transformer baselines in Table~\ref{tab2}, confirming that \(\mathcal{L}_{\mathrm{fold}}\) alone cannot drive near-zero folding; surface metrics also degrade (HD95 3.248\,mm, ASSD 0.763\,mm), the worst among all tested configurations. The volume-term-only configuration (\(\beta=0.02,\,\gamma=0\)) already suppresses the non-positive Jacobian ratio to \(1.90\times10^{-5}\) and reduces SDlogJ to 0.3343, with HD95 3.110\,mm and ASSD 0.750\,mm: \(\mathcal{L}_{\mathrm{volume}}\), by penalising compressive deformations across the entire interior mask, is the primary driver of both global distribution shaping and near-zero folding. Adding the fold hinge at the operating point (\(\beta=0.02,\,\gamma=20\)) further raises Dice to 0.7537, reduces SDlogJ to 0.3280, and yields the best ASSD (0.738\,mm) and a competitive HD95 (3.075\,mm) among all configurations, consistent with the complementary supervision rationale.

\paragraph{Coefficient sensitivity.}
A coarse two-axis validation sweep over \(\beta\) and \(\gamma\) on the held-out 58-subject validation set identified a broad optimum around \((\beta,\gamma)=(0.02,20)\), within which validation grouped Dice remained inside one standard deviation and the non-positive Jacobian ratio stayed below \(10^{-4}\).
The test-set ablation in Table~\ref{tab:ablation} confirms this robustness across all five metrics.
Among the four joint-regularisation configurations, grouped Dice varies by only 0.38 percentage points (0.7499--0.7537), and HD95 spans a narrow 3.045--3.075\,mm band, indicating that all four configurations reside on a flat optimum plateau rather than a sharp ridge.

\textit{Effect of lightening the volume weight.}
Reducing \(\beta\) from 0.02 to 0.01 (while halving \(\gamma\) from 20 to 10) produces the configuration \((\beta,\gamma)=(0.01,10)\), which raises the non-positive Jacobian ratio from \(1.46\times10^{-5}\) to \(5.56\times10^{-5}\) and worsens SDlogJ from 0.3280 to 0.3544.
Although Dice is competitive (0.7527), the weaker volume penalty allows more residual compression, and the weaker fold hinge leaves more headroom for borderline-folded voxels.
The operating point (\(\beta=0.02,\,\gamma=20\)) strictly dominates or ties this lighter configuration on all five metrics simultaneously.

\textit{Effect of a heavier fold weight (\(\gamma\) sweep at fixed \(\beta=0.02\)).}
Increasing \(\gamma\) from 20 to 50 while holding \(\beta=0.02\) produces marginal changes in regularity metrics (SDlogJ: \(0.3274\) vs.\ \(0.3280\); non-positive Jacobian ratio: \(2.06\times10^{-5}\) vs.\ \(1.46\times10^{-5}\)) but costs 0.33\,pp in Dice (\(0.7504\) vs.\ \(0.7537\)) and 0.013\,mm in ASSD (\(0.751\) vs.\ \(0.738\)).
The additional fold-hinge strength at \(\gamma=50\) therefore yields no meaningful regularity benefit beyond what \(\gamma=20\) already achieves, while introducing a small but consistent alignment penalty.
This confirms that the operating point is not perched at the edge of the regularisation cliff: \(\gamma=20\) is already in the saturation regime where further increasing the fold penalty does not materially reduce folding.

\textit{Effect of a heavier volume weight (\(\beta\) sweep at fixed regularity target).}
Raising \(\beta\) from 0.02 to 0.05 (with \(\gamma=10\)) pushes the non-positive Jacobian ratio from \(1.46\times10^{-5}\) to \(4.81\times10^{-6}\)---a factor of \({\sim}3\) improvement---and reduces SDlogJ from 0.3280 to 0.2909.
However, this comes at the cost of 0.38\,pp Dice (\(0.7499\) vs.\ \(0.7537\)) and 0.013\,mm ASSD (\(0.751\) vs.\ \(0.738\,\)mm).
Critically, both the operating point and the heavier-\(\beta\) variant have already suppressed the non-positive Jacobian ratio well below the practical threshold of \(10^{-4}\) used in our model-selection criterion; the additional factor-of-3 reduction in non-positive Jacobian ratio from \(1.46\times10^{-5}\) to \(4.81\times10^{-6}\) provides diminishing practical returns for morphometric workflows, since both values are negligible relative to voxel-grid resolution.

\paragraph{Operating-point selection rationale.}
We adopt \((\beta,\gamma)=(0.02,20)\) as the operating point based on a Pareto-optimality argument over Dice and non-positive Jacobian ratio, which are the two primary metrics for the target application---overlap-accurate, morphometry-safe registration.
The operating point simultaneously achieves the highest grouped Dice (0.7537) and the lowest ASSD (0.738\,mm) among all configurations, while maintaining a non-positive Jacobian ratio of \(1.46\times10^{-5}\)---more than two orders of magnitude below the \(10^{-4}\) threshold and the unregularised TransMorph baseline (\(1.50\times10^{-2}\)).
No other tested configuration strictly dominates the operating point on both Dice and non-positive Jacobian ratio simultaneously: the heavier-\(\beta\) variant \((0.05,10)\) improves non-positive Jacobian ratio further but sacrifices Dice, and all lighter configurations degrade at least one regularity metric.
The operating point therefore represents the allocation of regularisation budget that most efficiently converts additional training loss into alignment gain without leaving meaningful folding on the table.
The clamping constant \(\epsilon\) appears identically in both sub-terms; values in \([10^{-4},10^{-2}]\) produced visually indistinguishable validation curves and order-preserving metric rankings, so \(\epsilon=10^{-3}\) is not load-bearing for the reported conclusions.

\begin{table}[H]
\caption{Ablation study: term-isolation and coefficient-sensitivity runs on the 115-subject IXI test set. All runs use the same TransMorph backbone and training schedule as the operating point. \(\beta=0\) removes \(\mathcal{L}_{\mathrm{volume}}\); \(\gamma=0\) removes \(\mathcal{L}_{\mathrm{fold}}\). The row marked \((\dagger)\) is the operating point. HD95 and ASSD are computed with the grouped binary-mask method (17 VOI groups, union of constituent labels), consistent with the Dice grouping; absolute values therefore differ from the per-label method used in Table~\ref{tab2}.\label{tab:ablation}}
\centering
\scriptsize
\setlength{\tabcolsep}{2.5pt}
\begin{tabularx}{\textwidth}{Xccccccc}
\toprule
\textbf{Configuration} & \(\boldsymbol{\beta}\) & \(\boldsymbol{\gamma}\) & \textbf{Dice\(\uparrow\)} & \(\boldsymbol{\det(J_\phi)\le 0}\) \textbf{ratio\(\downarrow\)} & \textbf{SDlogJ\(\downarrow\)} & \textbf{HD95\(\downarrow\)} & \textbf{ASSD\(\downarrow\)}\\
\midrule
Fold hinge only (no \(\mathcal{L}_{\mathrm{volume}}\)) & 0.00 & 20 & 0.7503\(\pm\)0.0302 & \(4.69\times10^{-3}\pm1.50\times10^{-3}\) & 0.4738\(\pm\)0.0384 & 3.248\(\pm\)0.632 & 0.763\(\pm\)0.132\\
Volume term only (no \(\mathcal{L}_{\mathrm{fold}}\)) & 0.02 & 0 & 0.7510\(\pm\)0.0296 & \(1.90\times10^{-5}\pm9.07\times10^{-6}\) & 0.3343\(\pm\)0.0215 & 3.110\(\pm\)0.504 & 0.750\(\pm\)0.122\\
Combined (lighter) & 0.01 & 10 & \snd{0.7527\(\pm\)0.0270} & \(5.56\times10^{-5}\pm2.01\times10^{-5}\) & 0.3544\(\pm\)0.0234 & 3.060\(\pm\)0.470 & \snd{0.743\(\pm\)0.112}\\
Combined, operating point\(^\dagger\) & 0.02 & 20 & \best{0.7537\(\pm\)0.0275} & \snd{\(1.46\times10^{-5}\pm7.37\times10^{-6}\)} & 0.3280\(\pm\)0.0221 & 3.075\(\pm\)0.494 & \best{0.738\(\pm\)0.115}\\
Combined (heavier \(\gamma\))\(^\ddagger\) & 0.02 & 50 & 0.7504\(\pm\)0.0282 & \(2.06\times10^{-5}\pm7.84\times10^{-6}\) & \snd{0.3274\(\pm\)0.0207} & \snd{3.045\(\pm\)0.464} & 0.751\(\pm\)0.116\\
Combined (heavier \(\beta\)) & 0.05 & 10 & 0.7499\(\pm\)0.0272 & \(\bestm{4.81\times10^{-6}\pm4.47\times10^{-6}}\) & \best{0.2909\(\pm\)0.0184} & \best{3.045\(\pm\)0.439} & 0.751\(\pm\)0.110\\
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Values: mean\(\pm\)std over 115 IXI test subjects. \textcolor{BestColor}{\textbf{Bold red}}\,=\,best per metric; \textcolor{SndColor}{\underline{blue underline}}\,=\,second-best. Mean ties are broken by the smaller standard deviation (e.g., HD95 \(=3.045\,\)mm is shared by \((\beta,\gamma)=(0.05,10)\) with std \(0.439\) and \((0.02,50)\) with std \(0.464\); the former is marked best and the latter second-best). \(^\dagger\)Operating point checkpoint (\texttt{dsc0.743.pth.tar}); Dice/non\text{-}positive\text{-}ratio/SDlogJ are consistent with Table~\ref{tab2}. \(^\ddagger\)Trained for 100 epochs (rather than the 150 used for the other rows) after validation Dice plateaued; checkpoint \texttt{dsc0.741.pth.tar}.}}\end{table}
...
HypEReg-TransMorph & IXI & \best{0.7756 \(\pm\) 0.0300} & \snd{\(7.6\times10^{-5}\) \(\pm\) \(3.9\times10^{-5}\)} & \snd{0.3085 \(\pm\) 0.0109} & \best{2.4828 \(\pm\) 0.5253} & \best{0.7515 \(\pm\) 0.1080} \\
TransMorph & IXI & \snd{0.7691 \(\pm\) 0.0314} & 0.0096 \(\pm\) 0.0019 & 0.4703 \(\pm\) 0.0197 & \snd{2.6267 \(\pm\) 0.5207} & \snd{0.7832 \(\pm\) 0.1118} \\
TransMorphBayes & IXI & 0.7587 \(\pm\) 0.0352 & 0.0089 \(\pm\) 0.0019 & 0.4593 \(\pm\) 0.0186 & 2.7522 \(\pm\) 0.5553 & 0.8188 \(\pm\) 0.1194 \\
MIDIR & IXI & 0.7254 \(\pm\) 0.0291 & \best{0.0000 \(\pm\) 0.0000} & \best{0.2551 \(\pm\) 0.0123} & 2.8926 \(\pm\) 0.4944 & 0.9306 \(\pm\) 0.1079 \\
...
We evaluate this directly on the OASIS zero-shot setting. Note that Table~\ref{tab:oasis_zeroshot} follows the official 19 predefined Learn2Reg test pairs, whereas multi-atlas fusion is target-centric and therefore uses 20 unique OASIS test targets available for segmentation aggregation in our exported split. Six atlas subjects (IDs 50, 80, 150, 220, 300, 380) are held fixed across all models. For each of the 20 test targets, each atlas is registered to the target with each model; the 35-class OASIS atlas labels are propagated through the predicted displacement field using nearest-neighbour interpolation; and the six warped label maps are fused by majority voting \cite{heckemann2006majority}. Per-structure Dice between the fused label map and the target's ground-truth segmentation is reported, together with the fusion improvement \(\Delta\mathrm{Dice}=\mathrm{Dice}_{\mathrm{fusion}}-\overline{\mathrm{Dice}}_{\mathrm{single}}\), which isolates the gain attributable to label fusion from the baseline atlas--target overlap.
...
\midrule
HypEReg-TransMorph & \best{0.7795 \(\pm\) 0.0159} & \best{0.8271 \(\pm\) 0.0181} & +0.0477 & \best{0.8654} & \best{0.9056} & \best{0.9227} \\
TransMorph & \snd{0.7712 \(\pm\) 0.0146} & \snd{0.8201 \(\pm\) 0.0145} & \snd{+0.0489} & \snd{0.8492} & \snd{0.9006} & \snd{0.8979} \\
TransMorphBayes & 0.7597 \(\pm\) 0.0185 & 0.8058 \(\pm\) 0.0206 & +0.0460 & 0.8341 & 0.8975 & 0.8846 \\
MIDIR & 0.7161 \(\pm\) 0.0141 & 0.7696 \(\pm\) 0.0162 & \best{+0.0534} & 0.8238 & 0.8719 & 0.8955 \\
\bottomrule
\end{tabularx}
\noindent{\footnotesize{Values: mean\(\pm\)std over 20 OASIS test targets. \(N_{\mathrm{atlas}}=6\) atlas subjects (IDs: 50, 80, 150, 220, 300, 380; ID~80 substitutes for the originally planned ID~100, which is absent in the release split). Single Dice = per-atlas mean; Fused Dice = majority-voting result. \textcolor{BestColor}{\textbf{Bold red}}\,=\,best; \textcolor{SndColor}{\underline{blue underline}}\,=\,second-best.}}
\end{table}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\supplementary{Supplementary Materials (separate PDF) include: Table~S1 (IXI baseline checkpoint provenance), Table~S2 (auxiliary IXI deformation/surface metrics, including NSD@1\,mm), Tables~S3--S4 (IXI paired inferential summaries with Wilcoxon and BH-FDR), Table~S5 (grouped-VOI label mapping), Figure~S1 (descriptive IXI multi-metric bar-chart overview), Figure~S2 (qualitative registration comparison across three orthogonal views), OASIS retraining/environment notes, Table~S6 (OASIS-retrained paired Wilcoxon/BH-FDR; distinct from main-text zero-shot), Table~S7 (bootstrap 95\% confidence intervals on IXI subject-level means), optional notes on OASIS Jacobian-ROI and per-class multi-atlas CSV exports, implementation and profiling details, and additional qualitative registration/Jacobian visualizations.}
