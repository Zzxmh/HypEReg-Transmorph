...
Training settings for HypEReg-TransMorph are: Adam optimizer with AMSGrad \cite{kingma2014adam,reddi2018amsgrad}, learning rate \(1\times10^{-4}\), weight decay \(0\), batch size 2, and 500 epochs. The checkpoint-selection rule is best validation Dice. The current code path does not enforce a single fixed global random seed across all baselines, which is treated as a limitation. Hardware for the reported runs is NVIDIA RTX PRO 6000 Blackwell Workstation Edition (97,887 MiB VRAM), CUDA 12.8, and PyTorch 2.12.0.dev20260408+cu128 (nightly build used only for Blackwell-specific runtime; the same training and evaluation scripts are reproducible on the corresponding stable PyTorch release for that CUDA generation). Baselines are evaluated under the same test protocol; classical baselines use ANTs/ANTsPy and SimpleITK implementations \cite{avants2008ants,antspyx2024,simpleitk2013}.

\begin{table}[H]
\caption{Compact baseline provenance summary.\label{tab_baseline_repro}}
\begin{tabularx}{\textwidth}{lXXXX}
\toprule
\textbf{Family} & \textbf{Models} & \textbf{Source/training status} & \textbf{Main fairness limitation} & \textbf{Notes} \\
\midrule
TransMorph family & HypEReg-TransMorph, TransMorph, TransMorphBayes & Repository checkpoints; not retrained in this IXI-only manuscript & Upstream split provenance is partly unverifiable for some checkpoints & Unified atlas-to-subject evaluation scripts \\
Transformer-family baselines & CoTr, nnFormer, PVT & Repository checkpoints; not retrained & Adapted-backbone comparability and split-provenance uncertainty & Adapters use two-channel (moving/fixed) input and repository flow-output registration heads under shared IXI protocol \\
CNN/hybrid baselines & VoxelMorph-1, CycleMorph, MIDIR & Repository checkpoints; not retrained & Upstream split provenance partly unverifiable & Same preprocessing and metrics scripts \\
Classical baseline & SyN (ANTs) & Non-learning optimizer & Runtime/memory not directly comparable to feed-forward models & Evaluated under unified atlas-to-subject protocol \\
\bottomrule
\end{tabularx}
\end{table}

\subsection{Evaluation Metrics}
Primary metrics are Dice and Jaccard (higher is better), HD95 (mm) and ASSD (mm) (lower is better), and NSD@1\,mm \cite{dice1945,heimann2009,nikolov2018}. Deformation regularity is assessed by the non-positive Jacobian determinant ratio \(\#\{x:\det J_\phi(x)\le 0\}/\#\{x\}\), SDlogJ \(\left(\mathrm{std}(\log(\max(\det J_\phi,\epsilon)))\right)\), Jacobian quantiles (\(J_{\min}, J_{p01}, J_{p50}, J_{p99}, J_{\max}\)), bending energy, and mean absolute divergence \cite{bookstein1989tps,mok2020lapirn}. Intensity metrics include NCC/LNCC (window \(9^3\)), MI/NMI (64 bins), SSIM (dynamic range \(1.0\), adaptive odd window up to 11), and PSNR \cite{studholme1999nmi,wang2004ssim}.
...
\institutionalreview{Institutional Review Board Statement: Not applicable. This study performed secondary analysis of publicly available anonymized IXI data and did not recruit new human participants.}

\informedconsent{Informed Consent Statement: Not applicable to this secondary-analysis study using publicly available anonymized data.}

\dataavailability{Source code, configuration files, per-case metric tables, and figure/statistics scripts are openly available at \url{https://github.com/Zzxmh/TransMorph_Transformer_for_Medical_Image_Registration}. An archival Zenodo DOI will be assigned upon acceptance. The IXI dataset used in this study is publicly available at \url{https://brain-development.org/ixi-dataset/} (CC BY-SA 3.0). The preprocessed split follows the public TransMorph-style IXI preprocessing release.}
