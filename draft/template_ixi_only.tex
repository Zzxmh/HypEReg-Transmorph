...
\abstract{Accurate deformable brain MRI registration demands both high anatomical alignment and geometrically plausible deformation fields, yet contemporary Transformer-based methods routinely produce fields with folded voxels that violate the requirements of downstream analysis pipelines. We integrate hyperelastic-energy regularization (HypEReg)---a differentiable, plug-in training-loss term combining a volume-distortion penalty with an explicit anti-folding hinge---into a TransMorph-style dense registration backbone. On a held-out IXI atlas-to-subject protocol (115 test subjects), evaluated with grouped anatomical Dice, Jacobian-determinant statistics, and paired Wilcoxon/BH-FDR inferential testing, HypEReg-TransMorph achieves a mean grouped-structure Dice of 0.7537---comparable to the unregularized TransMorph baseline (0.7527)---while reducing the fraction of non-positive Jacobian voxels from 1.50\% to 0.0015\% and improving HD95 from 5.88\,mm to 3.16\,mm. The improvement in deformation regularity is statistically significant ($p < 10^{-19}$, FDR-adjusted $q < 10^{-19}$, rank-biserial $r=1.00$). Because HypEReg acts solely at training time, pure forward-pass inference cost remains identical to the baseline (0.0822\,s per volume, 5.685\,GB peak memory). These results demonstrate that explicit Jacobian-aware regularization can deliver near-diffeomorphic Transformer-based registration without compromising alignment accuracy or inference efficiency, with potential benefit for downstream applications including Jacobian-based morphometric analysis, atlas-based segmentation, treatment-planning support, and longitudinal disease monitoring.}
