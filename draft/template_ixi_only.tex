...
\abstract{Background: Deformable brain MRI registration requires accurate anatomical alignment while preserving geometrically plausible deformation fields. Methods: We integrated hyperelastic-energy regularization (HypEReg) into a TransMorph-style dense registration backbone to penalize implausible local volume changes and folding. The method was evaluated on a held-out IXI atlas-to-subject registration protocol (115 test subjects) using grouped anatomical labels, deformation-quality metrics based on the Jacobian determinant, and paired inferential statistical analysis (Wilcoxon/BH-FDR). Results: HypEReg-TransMorph achieved a mean grouped-structure Dice of 0.7537, comparable to the unregularized TransMorph baseline (0.7527), while reducing the fraction of non-positive Jacobian voxels from 1.50\% to 0.0015\% and improving HD95 from 5.88\,mm to 3.16\,mm. The improvement in deformation regularity was statistically significant under paired testing ($p < 10^{-19}$, FDR-adjusted $q < 10^{-19}$). These findings indicate that explicit Jacobian-aware regularization can enhance the geometric plausibility of dense deformation fields without compromising anatomical alignment accuracy or inference efficiency. Conclusions: HypEReg provides a practical regularization strategy for Transformer-based neuroimaging registration and may benefit downstream applications that depend on reliable deformation fields, including Jacobian-based morphometric analysis, atlas-based segmentation, treatment-planning support, and longitudinal disease monitoring.}
