...
The combined picture---confirmed quantitatively in Table~\ref{tab:ablation_terms}---is that \(\mathcal{L}_{\mathrm{volume}}\) drives the SDlogJ improvement (0.3159 for volume-only vs.\ 0.6808 for fold-only), \(\mathcal{L}_{\mathrm{fold}}\) drives the near-elimination of negative-Jacobian voxels (non-positive Jacobian ratio 0.000006 for volume-only vs.\ 0.002873 for fold-only), and the two terms act constructively: neither alone matches the Full operating point on all metrics simultaneously.

\subsection{Term-by-Term Ablation of HypEReg}\label{sec:ablation_terms}
We isolate the contribution of each additive HypEReg term at fixed preprocessing, backbone, and optimizer settings by training four variants: \emph{full} (volume + fold active at the IXI operating point with \(\alpha=0\)); \emph{volume-only} (\(\gamma=0\)); \emph{fold-only} (\(\beta=0\)); and \emph{length variant} (\(\alpha>0\) with \((\beta,\gamma)\) matched to the operating point). Results for the full, volume-only, and fold-only variants are populated in Table~\ref{tab:ablation_terms} from independent training runs on 115 held-out test subjects; the length-variant row is pending. The per-case metric exports follow the same evaluation protocol as Table~\ref{tab2} and are duplicated in Supplementary Table~S4.
...
\midrule
Full (volume+fold, \(\alpha{=}0\)) & \textbf{0.7537 \(\pm\) 0.0275} & \textbf{0.000015 \(\pm\) 0.000007} & \textbf{0.3280 \(\pm\) 0.0221} & \textbf{3.1561 \(\pm\) 0.5408} & \textbf{0.5891 \(\pm\) 0.1021} \\
Volume-only (\(\gamma{=}0\)) & 0.6147 \(\pm\) 0.0393 & \underline{0.000006 \(\pm\) 0.000003} & \underline{0.3159 \(\pm\) 0.0111} & \underline{4.7490 \(\pm\) 0.6681} & \underline{1.1092 \(\pm\) 0.2193} \\
Fold-only (\(\beta{=}0\)) & 0.5870 \(\pm\) 0.0373 & 0.002873 \(\pm\) 0.000587 & 0.6808 \(\pm\) 0.0101 & 4.9338 \(\pm\) 0.5909 & 1.2096 \(\pm\) 0.2096 \\
Length variant (\(\alpha{>}0\)) & --- & --- & --- & --- & --- \\
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Values: mean\(\pm\)std over 115 held-out IXI test subjects (\(n{=}115\)). Full-model Dice uses the 17-group bilateral method (Table~\ref{tab2}); ablation-variant Dice uses the 30-VOI comprehensive evaluation protocol (full model 30-VOI baseline: 0.7518 for calibration). Non-positive Jacobian ratio and SDlogJ from per-case Jacobian exports; HD95/ASSD from per-VOI surface-distance exports. Bold = best per metric; underline = second-best. Length-variant row pending.}
\end{table}
