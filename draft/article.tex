...
In this paper, we develop HypEReg, a non-linear hyperelastic regularizer derived from the Burger--Modersitzki--Ruthotto (BMR) energy \cite{burger2013hyperelastic} but redesigned for stochastic optimization in a deep-learning context. HypEReg combines a clamped-rational volume-distortion penalty \((\det J_\phi-1)^2/\max(\det J_\phi,\epsilon)\) with an explicit per-voxel anti-folding hinge \([\max(0,\epsilon-\det J_\phi)]^2\), both expressed directly as functions of the Jacobian determinant of the predicted displacement field. The clamped-rational form is bounded under stochastic gradient descent, asymmetric in \(v\leftrightarrow 1/v\) so that compressive deformations remain more strongly penalized than expansive ones, and non-linear in \(\det J_\phi\) and therefore remains well-conditioned at moderate-to-large local strains under the \(\epsilon\)-clamped formulation. The hinge term is differentiable everywhere and supplies a per-voxel quadratic supervision signal wherever a fold is incipient. We integrate this regularizer into a TransMorph backbone as a parameter-free, loss-side module: training is modified, but the inference graph, runtime, and parameter file format are identical to those of unregularized TransMorph.

The contributions of this study are as follows.
\begin{itemize}
\item \emph{A non-linear hyperelastic regularizer for learned deformable registration.} HypEReg combines a clamped-rational volume term and an anti-folding hinge that act directly and non-linearly on \(\det J_\phi\). The formulation remains well-conditioned across moderate-to-large local strains, provides a per-voxel quadratic supervision signal on candidate folded voxels, and differs from the conformal-invariant hyperelastic energy of \citet{zou2023conformal}, the BMR-style volume-change penalty of \citet{hering2021lung}, and the selective Jacobian-determinant penalty of \citet{mok2020fast}---the last of which contributes only the hinge component.
...
\item \emph{IXI\(\to\)OASIS zero-shot cross-cohort generalization and downstream clinical validation.} All IXI-trained checkpoints are evaluated directly on OASIS Learn2Reg-style test pairs without any fine-tuning. HypEReg-TransMorph achieves the highest Dice and lowest non-positive Jacobian ratio in the zero-shot group (Section~\ref{sec:oasis_zs}; Table~\ref{tab:oasis_zeroshot}). The regularity advantage cascades into downstream clinical workflows: multi-atlas label fusion yields the highest fused Dice (Section~\ref{sec:multiatlas}; Table~\ref{tab:multiatlas}), and Jacobian-integration volumetric reliability analysis shows the lowest ARE and highest ICC for hippocampal and ventricular volumes (Section~\ref{sec:volreliability}; Table~\ref{tab:volreliability}).
\end{itemize}

The remainder of the paper is organized as follows. Section~\ref{sec:methods} describes the data, the backbone, the proposed regularizer, and the evaluation protocol. Section~\ref{sec:results} reports the IXI benchmark, the HypEReg term-design rationale, the IXI\(\to\)OASIS zero-shot evaluation, downstream multi-atlas fusion, volumetric reliability, and runtime profiling. Section~\ref{sec:discussion} discusses the position of HypEReg within the deep-registration regularization literature, its clinical relevance, and the principal limitations of the present study, and Section~\ref{sec:conclusion} concludes.
...
To probe cohort shift beyond IXI, we adopt the OASIS preprocessed Learn2Reg-style release bundled with the OASIS evaluation utilities of the released repository. Training reuses the TransMorph backbone configuration and the HypEReg operating point \((\beta,\gamma,\epsilon)=(0.02,20,10^{-3})\) employed on IXI, applied to the OASIS train/validation subject splits without modification. Evaluation uses 19 released test pairs, with displacement fields exported at half resolution and evaluated using the same metric pipeline as the IXI benchmark (Dice, surface distances, NMI, LNCC, SSIM, and Jacobian summaries). Classical SyN (ANTs) and affine mutual-information baselines run through the same adapter interface as on IXI; published or retrained checkpoints for CNN/Transformer baselines are used where available and otherwise fall back to the corresponding IXI-trained checkpoints for zero-shot evaluation. Paired significance testing of HypEReg-TransMorph against all other models is performed on per-case metric exports using the same Wilcoxon + BH-FDR procedure as the IXI benchmark.
...
+
\beta \mathcal{L}_{\mathrm{volume}} + \gamma \mathcal{L}_{\mathrm{fold}}.
\end{equation}
\end{linenomath}
...
\subsection{Design Rationale of HypEReg Terms and Hyperparameter Sensitivity}\label{sec:hypereg_roles}
...
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Values are reported as mean \(\pm\) standard deviation over 115 test subjects. Dice is the grouped mean over 17 anatomical structure groups following the \texttt{analysis\_trans.py} protocol; \(\det(J_\phi)\le 0\) ratio, SDlogJ, HD95, and ASSD are taken from per-case evaluation exports. Bold indicates best value per metric; underline indicates second-best.}
\end{table}
...
Results are summarized in Table~\ref{tab:multiatlas}. HypEReg-TransMorph achieves the highest fused Dice (\(0.8271\pm0.0181\)) in the zero-shot group, with \(\Delta\mathrm{Dice}=+0.0477\). By comparison, plain TransMorph attains a fused Dice of \(0.8201\pm0.0145\) with \(\Delta\mathrm{Dice}=+0.0489\), and TransMorphBayes attains \(0.8057\pm0.0200\) with \(\Delta\mathrm{Dice}=+0.0460\). The fact that HypEReg-TransMorph achieves the highest absolute fused Dice despite a slightly smaller \(\Delta\mathrm{Dice}\) than TransMorph reflects its higher single-atlas baseline (\(0.7795\) vs.\ \(0.7712\)): HypEReg starts from a geometrically more consistent registration, so fusion provides a smaller marginal gain but reaches a higher absolute quality. Per-ROI breakdown shows HypEReg-TransMorph leading on hippocampus (\(0.8654\)) and thalamus (\(0.9227\)) fused Dice, the two structures most sensitive to local folding artefacts.
...
\midrule
HypEReg-TransMorph & \underline{0.7795 \(\pm\) 0.0159} & \textbf{0.8271 \(\pm\) 0.0181} & +0.0477 & \textbf{0.8654 \(\pm\) 0.0179} & \textbf{0.9056 \(\pm\) 0.0534} & \textbf{0.9227 \(\pm\) 0.0123} \\
TransMorph & 0.7712 \(\pm\) 0.0146 & \underline{0.8201 \(\pm\) 0.0145} & \textbf{+0.0489} & \underline{0.8492 \(\pm\) 0.0215} & \underline{0.9006 \(\pm\) 0.0475} & \underline{0.8979 \(\pm\) 0.0101} \\
TransMorphBayes & 0.7596 \(\pm\) 0.0181 & 0.8057 \(\pm\) 0.0200 & +0.0460 & 0.8346 \(\pm\) 0.0200 & 0.8973 \(\pm\) 0.0540 & 0.8841 \(\pm\) 0.0158 \\
SyN (ANTs) & --- & --- & --- & --- & --- & --- \\
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Values: mean\(\pm\)std over 20 OASIS test targets. \(N_{\mathrm{atlas}}=5\) (IDs: 50, 80, 150, 220, 300, 380; atlas 80 replaces the missing ID 100). Single Dice = per-atlas mean; Fused Dice = majority-voting result. Bold\,=\,best; underline\,=\,second-best. SyN omitted due to per-pair iterative cost. Results: \texttt{scripts/oasis\_downstream.py}.}
\end{table}

\subsection{Volumetric Reliability on OASIS}\label{sec:volreliability}
Region-wise volume estimates derived from a registration field are the canonical input to tensor-based morphometry and to atrophy-tracking biomarkers in neurodegenerative disease. Two estimation paths are common: direct voxel counting on atlas-propagated segmentation labels (\(V_{\mathrm{prop}}\)), and integration of \(\det J_\phi\) over the atlas-space ROI (\(V_{\mathrm{jac}}\)). The latter path is directly affected by Jacobian-determinant regularity and provides the most stringent downstream test of HypEReg's value.

For each of the 20 OASIS test subjects we compute the reference volume of three bilateral clinical ROIs (hippocampi, lateral ventricles, and thalami) from the ground-truth segmentation. The propagation estimate \(V_{\mathrm{prop}}\) uses the multi-atlas-fused label map from Section~\ref{sec:multiatlas}; the Jacobian-integration estimate \(V_{\mathrm{jac}}\) is the mean of six per-atlas integrals \(\int_{\mathrm{ROI}_j}\det J_\phi\,\mathrm{d}x\). Reliability is quantified by absolute relative error (ARE) and intraclass correlation ICC(3,1). Results are summarized in Table~\ref{tab:volreliability}.

\begin{table}[H]
\caption{ROI volumetric reliability on OASIS zero-shot transfer (\(n=20\) test targets). ARE\,=\,absolute relative error (\%) vs.\ ground-truth voxel count; ICC\,=\,ICC(3,1) absolute agreement. Sub-scripts: \textit{prop}\,=\,atlas-propagation path; \textit{jac}\,=\,Jacobian-integration path.\label{tab:volreliability}}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{llcccc}
\toprule
\textbf{ROI} & \textbf{Model} & \textbf{ARE\(_{\mathrm{prop}}\) (\%)\(\downarrow\)} & \textbf{ARE\(_{\mathrm{jac}}\) (\%)\(\downarrow\)} & \textbf{ICC\(_{\mathrm{prop}}\)\(\uparrow\)} & \textbf{ICC\(_{\mathrm{jac}}\)\(\uparrow\)}\\
\midrule
\multirow{3}{*}{Hippocampus}
 & HypEReg-TransMorph & \textbf{8.5 \(\pm\) 6.0} & \textbf{11.7 \(\pm\) 12.8} & 0.71 & 0.67 \\
 & TransMorph & \underline{8.8 \(\pm\) 5.9} & \underline{12.1 \(\pm\) 13.1} & \underline{0.77} & \underline{0.70} \\
 & TransMorphBayes & 10.6 \(\pm\) 6.3 & 15.0 \(\pm\) 14.3 & \textbf{0.87} & \textbf{0.86} \\
\midrule
\multirow{3}{*}{Lateral Ventricles}
 & HypEReg-TransMorph & \textbf{10.9 \(\pm\) 11.0} & \dag & \textbf{n/a} & \underline{0.91} \\
 & TransMorph & \underline{11.3 \(\pm\) 9.1} & \dag & n/a & \textbf{0.91} \\
 & TransMorphBayes & 14.0 \(\pm\) 11.1 & \dag & n/a & 0.90 \\
\midrule
\multirow{3}{*}{Thalamus}
 & HypEReg-TransMorph & \textbf{2.6 \(\pm\) 2.3} & \underline{14.9 \(\pm\) 8.4} & n/a & n/a \\
 & TransMorph & \underline{3.4 \(\pm\) 1.8} & \textbf{13.4 \(\pm\) 7.6} & n/a & \underline{0.62} \\
 & TransMorphBayes & 2.5 \(\pm\) 2.1 & 13.3 \(\pm\) 7.4 & n/a & 0.34 \\
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize ARE (\%) = absolute relative error vs.\ ground-truth voxel count, mean\(\pm\)std over 20 test targets. ICC = ICC(3,1), absolute agreement. Bold\,=\,best; underline\,=\,second-best per column (where applicable). \dag: Lateral-ventricle ARE\(_{\mathrm{jac}}\)\,>\,100\% for all models (ventricle size varies strongly across atlases; Jacobian-integration path is unreliable for this ROI). n/a: ICC\,<\,0 (no reliable agreement above chance). SyN omitted. \textit{prop}\,=\,atlas-propagation; \textit{jac}\,=\,Jacobian-integration.}
\end{table}
...
The downstream multi-atlas and volumetric experiments in Sections~\ref{sec:multiatlas} and~\ref{sec:volreliability} close the gap between registration-level metrics and clinical-level evidence. HypEReg-TransMorph achieves the highest fused Dice (0.8271) in Table~\ref{tab:multiatlas}, reflecting geometric consistency: fold-free warps produce less ambiguous label assignments at each voxel, raising the majority-vote margin. The propagation-based hippocampal ARE is also lowest (8.5\%), confirming that the fold-suppression benefit extends to volumetric estimation. The Jacobian-integration path for hippocampus yields ARE of 11.7\% (vs.\ 12.1\% for TransMorph and 15.0\% for TransMorphBayes), consistent with the direct Jacobian-determinant improvements documented in Table~\ref{tab:oasis_zeroshot}. For lateral ventricles, Jacobian integration is unreliable for all models (ARE\,>\,100\%) owing to large inter-atlas size variation; this limitation is common to Jacobian-based morphometry in highly deformable structures and is not specific to HypEReg.
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\supplementary{Supplementary Materials (separate PDF) include: Table~S1 (inferential summary for selected baselines), Table~S2 (paired inferential statistics with Wilcoxon+BH-FDR), Table~S3 (grouped-VOI label mapping), Figure~S1 (descriptive IXI multi-metric overview), OASIS training/environment details, Table~S5 (OASIS paired Wilcoxon/BH-FDR), Table~S6 (bootstrap 95\% confidence intervals on subject-level metrics), Table~S7 (OASIS Jacobian ROI cleanliness analysis), Table~S8 (LPBA40 zero-shot evaluation), implementation and profiling details, and additional qualitative registration/Jacobian visualizations.}
