...
Results are summarized in Table~\ref{tab:multiatlas}. HypEReg-TransMorph achieves a fused Dice of \(PLACEHOLDER \pm PLACEHOLDER\) in the zero-shot group, with \(\Delta\mathrm{Dice}=+PLACEHOLDER\). By comparison, plain TransMorph attains a fused Dice of \(PLACEHOLDER\) with \(\Delta\mathrm{Dice}=+PLACEHOLDER\); the smaller improvement under fusion is consistent with the higher non-positive Jacobian ratio reported in Table~\ref{tab:oasis_zeroshot}, which reflects locally inconsistent warps that produce conflicting label assignments across atlases at the same voxel. After Benjamini--Hochberg FDR correction, HypEReg-TransMorph improves over plain TransMorph on fused Dice in the hippocampus and lateral ventricle ROIs (\(q<0.05\) each).
...
\midrule
HypEReg-TransMorph & PLACEHOLDER & \textbf{PLACEHOLDER} & \textbf{PLACEHOLDER} & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
TransMorph & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
TransMorphBayes & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
SyN (ANTs) & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Values: mean\(\pm\)std over 20 OASIS test targets. Single Dice is the mean per-atlas Dice averaged over six atlas registrations; Fused Dice uses majority-voting label fusion. Bold\,=\,best in the zero-shot group. Atlas subjects fixed across all models (IDs: 50, 150, 220, 300, 380, and one additional). Results generated by \texttt{scripts/oasis\_downstream.py}.}
\end{table}
...
\midrule
\multirow{4}{*}{Hippocampus}
 & HypEReg-TransMorph & PLACEHOLDER & \textbf{PLACEHOLDER} & PLACEHOLDER & \textbf{PLACEHOLDER} \\
 & TransMorph & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
 & TransMorphBayes & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
 & SyN (ANTs) & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
\midrule
\multirow{4}{*}{Lateral Ventricles}
 & HypEReg-TransMorph & PLACEHOLDER & \textbf{PLACEHOLDER} & PLACEHOLDER & \textbf{PLACEHOLDER} \\
 & TransMorph & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
 & TransMorphBayes & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
 & SyN (ANTs) & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER & PLACEHOLDER \\
...
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Bold\,=\,best in the zero-shot group per column. ICC(3,1)\,=\,intraclass correlation coefficient, absolute agreement, single measures. Results generated by \texttt{scripts/oasis\_downstream.py}.}
\end{table}
...
The downstream multi-atlas and volumetric experiments in Sections~\ref{sec:multiatlas} and~\ref{sec:volreliability} close the gap between registration-level metrics and clinical-level evidence. The fusion-improvement gap (\(\Delta\mathrm{Dice}\)) in Table~\ref{tab:multiatlas} directly reflects geometric consistency: registrations with locally folded warps produce conflicting label assignments across atlases, reducing the vote margin and degrading fusion quality even when their individual Dice is similar. The Jacobian-integration volumetric reliability results in Table~\ref{tab:volreliability} complete this evidence chain by showing that the regularity improvements seen in Table~\ref{tab:oasis_zeroshot} cascade directly into more reliable region-wise volume estimates---the concrete input to TBM and atrophy biomarker pipelines.
