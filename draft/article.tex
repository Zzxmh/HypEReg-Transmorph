...
The remainder of the paper is organized as follows. Section~\ref{sec:methods} describes the data, the backbone, the proposed regularizer, and the evaluation protocol. Section~\ref{sec:results} reports the IXI benchmark, the HypEReg term-design rationale, the IXI\(\to\)OASIS zero-shot cross-cohort evaluation, and runtime profiling. Section~\ref{sec:discussion} discusses the position of HypEReg within the deep-registration regularization literature, its clinical relevance, and the principal limitations of the present study, and Section~\ref{sec:conclusion} concludes.
