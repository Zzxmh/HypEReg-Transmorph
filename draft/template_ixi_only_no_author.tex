...
\corres{Correspondence: anonymized for review}
...
\abstract{Deformable brain MRI registration requires both accurate anatomical alignment and geometrically plausible deformation fields, yet Transformer-based methods routinely generate locally folded deformations that invalidate downstream Jacobian-based morphometry. We introduce HypEReg, a non-linear hyperelastic regularizer that couples a clamped-rational volume-distortion term $(\det J_\phi-1)^2/\max(\det J_\phi,\epsilon)$ with an explicit per-voxel anti-folding hinge $[\max(0,\epsilon-\det J_\phi)]^2$, and integrate it as a loss-side, parameter-free module into a TransMorph dense-flow backbone (HypEReg-TransMorph). Unlike second-order linear-elastic regularizers, which control folding only through an asymptotic small-strain bound, HypEReg supplies direct per-voxel supervision on $\det J_\phi$ and is mathematically valid at arbitrary local strain. On the standard IXI atlas-to-subject protocol (115 held-out test subjects), HypEReg-TransMorph reduces the non-positive Jacobian-determinant ratio by approximately three orders of magnitude relative to TransMorph (1.502\% $\to$ 0.0015\%, $p<10^{-19}$ paired Wilcoxon, BH-FDR $q<10^{-19}$, rank-biserial $r=1.00$), reduces SDlogJ from 0.5064 to 0.3280, and reduces HD95 from 5.88 to 3.16\,mm and ASSD from 1.56 to 0.59\,mm, while preserving grouped-VOI Dice (0.7537 vs.\ 0.7527). Because HypEReg acts only during training, forward inference cost is identical to the unregularized TransMorph baseline (0.0822\,s/volume, 5.685\,GB peak memory, 46.77\,M parameters). On a matched IXI 403/58/115 split, an indirect comparison with the recent linear-elastic ElasticMorph framework shows that the two regularizers occupy complementary regions of the accuracy/regularity Pareto front, with HypEReg specifically optimized for near-elimination of negative-Jacobian voxels. The results support non-linear, Jacobian-aware regularization as a low-cost route to near-diffeomorphic, clinically deployable Transformer-based deformable registration.}
...
\subsection{Hyperelastic-Term Roles and Hyperparameter Sensitivity}\label{sec:ablation}
...
\end{itemize}
Within this positioning, a controlled head-to-head retraining of both regularizers on bit-identical hardware/data/optimizer schedules, with repeated seeds and on a second cohort (e.g.\ LPBA40, OASIS, or Mindboggle), is the natural experimental complement to the present indirect comparison; we identify it as a follow-up study (Section~\ref{sec:limitations}) but emphasize that it would resolve the relative \emph{magnitude} of the two effects rather than the qualitative finding that linear-elastic and hyperelastic-Jacobian regularization populate distinct regions of the accuracy/regularity trade-off.
...
We deliberately scope the present manuscript to a single-cohort, single-protocol IXI atlas-to-subject benchmark with subject-level paired inferential statistics, and we acknowledge three concrete limitations that define the immediate experimental roadmap. (i) \emph{Single training seed.} Following the convention of the upstream TransMorph reference, the released training pipeline does not pin a global random seed; statistical inference is therefore performed across the 115 held-out test subjects rather than across multiple training seeds. A repeat-with-seeds extension ($\geq 3$ independent re-trainings of HypEReg-TransMorph and the strongest baselines, with paired Wilcoxon tests aggregated over both subjects and seeds) is the most informative next step and is supported by the released training scripts. (ii) \emph{Indirect rather than direct comparison with ElasticMorph.} The IXI 403/58/115 protocol is matched between this work and \cite{elasticmorph2025}, allowing the relative-improvement comparison reported in Section~\ref{sec:elasticmorph_indirect}, but a controlled head-to-head retraining of both regularizers on bit-identical hardware/data/optimizer schedules has not yet been performed; we identify it as the highest-value follow-up because it would resolve the relative magnitude of the two effects observed in Table~\ref{tab:em_indirect}. (iii) \emph{Single dataset and single anatomy.} All reported metrics are computed on IXI brain MRI; cross-cohort generalization to LPBA40 (used by ElasticMorph), OASIS, and Mindboggle---and cross-anatomy extension to lung CT (used by \cite{hering2021lung,zou2023conformal}), abdominal MR/CT, and inter-subject cardiac registration---is a natural next study. The training and evaluation code released with this manuscript is dataset-agnostic at the level of its loss and metric implementations, so adding a second cohort requires only the corresponding preprocessing pipeline.

Beyond these direct extensions, future work will pursue multi-modal registration, pathology-rich cohorts, uncertainty-aware registration frameworks, and adaptive (spatially varying) hyperelastic weighting to better balance local accuracy against topology constraints under heterogeneous anatomical deformations. Beyond methodological extensions, prospective clinical validation on disease-specific cohorts (e.g.\ ADNI for Alzheimer's progression, OASIS for ageing, BraTS for tumour-bearing brains) will be needed to quantify how the observed regularity gains translate into improved sensitivity of derived biomarkers and reduced reader workload in real reporting workflows.

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Conclusions}

HypEReg-TransMorph introduces a non-linear hyperelastic regularizer---a clamped-rational volume-distortion term coupled with an explicit anti-folding hinge---into a Transformer-based dense-flow registration backbone, and demonstrates on the IXI atlas-to-subject benchmark that physically motivated Jacobian-determinant control can be added to learned registration without any inference-time cost. Under the standard 115-subject test protocol, HypEReg-TransMorph reduces the non-positive Jacobian-determinant ratio by approximately three orders of magnitude relative to TransMorph and TransMorphBayes (1.502\% $\to$ 0.0015\%), reduces SDlogJ from 0.5064 to 0.3280, and reduces HD95 from 5.88\,mm to 3.16\,mm, while preserving grouped-Dice overlap and incurring zero added inference latency or parameter count (Table~\ref{tab3}). An indirect comparison on the matched IXI 403/58/115 protocol shows that HypEReg and the recent linear-elastic ElasticMorph framework occupy complementary regions of the accuracy/regularity Pareto front (Section~\ref{sec:elasticmorph_indirect}), with HypEReg specifically optimized for near-elimination of negative-Jacobian voxels.

From a clinical neuroimaging perspective, this gain directly addresses a long-standing usability barrier of learning-based registration: Jacobian-based biomarkers, atlas-fusion segmentation, and longitudinal morphometry pipelines all assume one-to-one, fold-free warps, an assumption that unconstrained Transformer backbones routinely violate. By delivering near-diffeomorphic fields at feed-forward inference speed and with no change to the deployment architecture, HypEReg-TransMorph offers a drop-in upgrade path for brain-MRI research and clinical-research workflows---spanning atrophy quantification in neurodegenerative disease, atlas-based delineation for radiotherapy and surgical planning, and population-scale morphometry---without sacrificing the throughput and reproducibility advantages that motivated the move from classical iterative methods to learned registration in the first place. The released open-source package, the indirect quantitative comparison with ElasticMorph, and the explicitly defined experimental roadmap (multi-seed retraining, cross-cohort and cross-anatomy validation, head-to-head against linear-elastic regularization) together position non-linear Jacobian-aware regularization as a practical, low-cost building block for clinically deployable Transformer-based deformable registration.
