# -*- coding: utf-8 -*-
"""nnFormer (IXI-trained) — zero-shot transfer to OASIS."""
...
def build_model(device: str = "cuda"):
    ...
    from nnFormer.Swin_Unet_l_gelunorm import swintransformer as nnFormer  # noqa: WPS433

    ckpt = _ckpt_path()
    if not os.path.isfile(ckpt):
        raise FileNotFoundError(f"nnformer_zs_oasis: nnFormer IXI ckpt missing: {ckpt}")
    model = nnFormer()
    model.load_state_dict(load_state_dict_any(ckpt))
    print(f"nnformer_zs_oasis: loading IXI ckpt {ckpt}", flush=True)
