# -*- coding: utf-8 -*-
"""CoTr (IXI-trained) — zero-shot transfer to OASIS."""
...
def build_model(device: str = "cuda"):
    ...
    from CoTr.network_architecture.ResTranUnet import ResTranUnet as CoTr  # noqa: WPS433

    ckpt = _ckpt_path()
    if not os.path.isfile(ckpt):
        raise FileNotFoundError(f"cotr_zs_oasis: CoTr IXI ckpt missing: {ckpt}")
    model = CoTr()
    model.load_state_dict(load_state_dict_any(ckpt))
    print(f"cotr_zs_oasis: loading IXI ckpt {ckpt}", flush=True)
