...
\noindent This document accompanies the manuscript \emph{HypEReg: Hyperelastic Regularization for Reducing Folding in Transformer-Based Brain MRI Registration}. Extended quantitative results, inferential tables (S1--S2), grouped-VOI mapping (S3), Figure~S1, OASIS protocol notes with Tables~S4--S5, and implementation/profiling details are provided here as separate Supplementary Materials (they are not duplicated in the main journal PDF).
...
\noindent Reference model: \texttt{transmorph\_her\_oasis} (HER-TransMorph, OASIS-retrained). All pairs $n{=}19$. Only significant contrasts ($q_{\mathrm{BH}}<0.05$) shown; sign of direction: $+$ = HER-TransMorph better.

\begin{table}[H]
\caption{Significant paired Wilcoxon contrasts (BH-FDR, $q<0.05$) for HER-TransMorph vs baselines on 19 OASIS test pairs.\label{tab:s5_oasis_wilcoxon}}
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{llXXX}
\toprule
\textbf{Baseline} & \textbf{Metric} & \textbf{HER mean} & \textbf{Baseline mean} & \textbf{$p$ / $q$} \\
\midrule
...
\multirow{5}{*}{MIDIR} & dice\_mean\(\uparrow\) & 0.7966 & 0.7254 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ \\
 & non\_jec\(\downarrow\) & $1.0\times10^{-5}$ & 0.0000 & $1.3\times10^{-4}$ / $1.3\times10^{-4}$ (\emph{HER worse}) \\
 & SDlogJ\(\downarrow\) & 0.2786 & 0.2551 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ (\emph{HER worse}) \\
 & HD95\_mean\(\downarrow\) & 2.294 & 2.893 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ \\
 & ASSD\_mean\(\downarrow\) & 0.680 & 0.931 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ \\
\midrule
\multirow{5}{*}{TransMorph (dsc0.857)} & dice\_mean\(\uparrow\) & 0.7966 & 0.8610 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ (\emph{HER worse}) \\
 ...
 & HD95\_mean\(\downarrow\) & 2.294 & 1.598 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ (\emph{HER worse}) \\
 & ASSD\_mean\(\downarrow\) & 0.680 & 0.483 & $3.8\times10^{-6}$ / $3.8\times10^{-6}$ (\emph{HER worse}) \\
\midrule
...
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize $p$: two-sided Wilcoxon signed-rank; $q$: Benjamini--Hochberg FDR. ``HER worse'' indicates the signed direction favours the baseline on that metric; these contrasts remain significant indicating MIDIR/dsc0.857 significantly outperform HER-TransMorph on those regularity/overlap dimensions. Full output in \texttt{OASIS/Eval\_Results/\_stats/oasis\_pairwise\_wilcoxon.csv}.}
\end{table}
