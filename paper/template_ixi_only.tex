...
\Title{HypEReg: Hyperelastic Regularization for Reducing Folding in Transformer-Based Brain MRI Registration}
...
\abstract{We study whether Jacobian-aware hyperelastic regularization generalizes beyond a single training cohort and how each HypEReg loss term contributes at fixed operating-point weights. HypEReg couples a clamped-rational volume-distortion term $(\det J_\phi-1)^2/\max(\det J_\phi,\epsilon)$ with an explicit per-voxel anti-folding hinge $[\max(0,\epsilon-\det J_\phi)]^2$ as a loss-side, parameter-free module in a TransMorph dense-flow backbone (HypEReg-TransMorph). A systematic term-by-term ablation (full objective vs.\ volume-only vs.\ fold-only vs.\ a length-term variant) is specified with per-case metric exports; numerical rows are completed from an independent training server and summarized in Table~\ref{tab:ablation_terms} and Supplementary Table~S4. For cross-cohort validation, we retrain HypEReg-TransMorph on OASIS with the same loss weights as the primary IXI run and evaluate on the released Learn2Reg-style OASIS test pairs using the same metric panel and paired Wilcoxon/BH-FDR machinery as on IXI (Table~\ref{tab:oasis_main}; Supplementary Table~S5). On the standard IXI atlas-to-subject protocol (115 held-out test subjects), HypEReg-TransMorph reduces the non-positive Jacobian-determinant ratio by approximately three orders of magnitude relative to TransMorph (1.502\% $\to$ 0.0015\%, $p<10^{-19}$ paired Wilcoxon, BH-FDR $q<10^{-19}$, rank-biserial $r=1.00$), reduces SDlogJ from 0.5064 to 0.3280, and reduces HD95 from 5.88 to 3.16\,mm and ASSD from 1.56 to 0.59\,mm, while preserving grouped-VOI Dice (0.7537 vs.\ 0.7527). Forward inference cost matches the unregularized TransMorph baseline (0.0822\,s/volume, 5.685\,GB peak memory, 46.77\,M parameters). An indirect comparison on the matched IXI 403/58/115 split positions HypEReg relative to the recent linear-elastic ElasticMorph framework on the accuracy/regularity Pareto front. Together, two-cohort evaluation and explicit ablation hooks support non-linear Jacobian-aware regularization as a reproducible, low-cost route toward near-diffeomorphic Transformer-based deformable registration.}

% Keywords
\keyword{medical image registration; deformable registration; transformer; hyperelastic-energy regularization; HypEReg-TransMorph; folding suppression; Jacobian regularization; Jacobian determinant; brain MRI; OASIS; cross-cohort validation; ablation study}
...
\item \emph{A unified IXI benchmark with subject-level paired inference and an indirect literature comparison.} On 115 held-out IXI atlas-to-subject pairs we benchmark HypEReg-TransMorph against five Transformer-family models (TransMorph, TransMorphBayes, CoTr, nnFormer, PVT), three CNN/hybrid models (VoxelMorph, CycleMorph, MIDIR), and the classical SyN reference, using paired Wilcoxon signed-rank tests with Benjamini--Hochberg FDR control on subject-level metrics. We further provide a quantitative indirect comparison with the recent linear-elastic ElasticMorph framework \cite{elasticmorph2025} on a matched 403/58/115 IXI split (Section~\ref{sec:elasticmorph_indirect}).
\item \emph{Cross-cohort OASIS retraining and paired evaluation.} HypEReg-TransMorph is retrained on OASIS with the same HypEReg weights as the primary IXI experiment; predictions on the released Learn2Reg-style OASIS test pairs are scored with the same metric code path as IXI, and paired Wilcoxon/BH-FDR summaries are exported alongside per-case CSVs (Section~\ref{sec:oasis_xcohort}; Supplementary Table~S5).
...
Jacobian-aware and hyperelastic penalties provide direct control of local volume change and folding behavior \cite{burger2013hyperelastic}, complementing first-order smoothness regularizers by explicitly penalizing singular or near-singular deformation regions, which is important for downstream morphometric interpretation. The original Burger--Modersitzki--Ruthotto (BMR) formulation combines length, surface (cofactor), and volume terms inside a polyconvex energy and proves existence of minimizers in a discretize-then-optimize variational setting; HypEReg retains the volume-control behavior of this energy through a redesigned, finite-gradient rational form and adds an explicit anti-folding hinge in place of the unbounded BMR fold-prevention asymptote, with the precise mathematical correspondence stated in Section~\ref{sec:loss}.
...
HypEReg-TransMorph occupies a distinct point in this design space along three axes. (i) \emph{Locus of regularization.} Rather than imposing a small-strain elastic residual on the displacement field~\cite{elasticmorph2025} or a conformal-invariant hyperelastic energy~\cite{zou2023conformal}, HypEReg operates directly on $\det J_\phi$, the geometric quantity used downstream for tensor-based morphometry; this makes the regularizer formally independent of the strain magnitude and complementary to (rather than redundant with) first-order smoothness penalties such as $\mathcal{L}_{\mathrm{grad}}$. (ii) \emph{Folding control.} ElasticMorph~\cite{elasticmorph2025} guarantees folding suppression only asymptotically, via a small-strain upper bound on $\#\{\det J_\phi\le 0\}$; HypEReg adds an explicit anti-folding hinge that supplies a per-voxel quadratic penalty on every voxel with $\det J_\phi\le\epsilon$, which empirically reduces the non-positive Jacobian ratio by three orders of magnitude (Section~\ref{sec:results}). (iii) \emph{Backbone family.} Earlier hyperelastic and volume-change regularizers were demonstrated on CNN backbones~\cite{mok2020fast,hering2021lung,zou2023conformal} or on point-cloud / mesh-based pipelines; HypEReg is, to our knowledge, the first non-linear hyperelastic-subset regularizer evaluated on a Transformer-based dense-flow backbone with subject-level paired Wilcoxon/BH-FDR inferential statistics. We give the precise differences in formulation in Section~\ref{sec:loss} and quantitatively position the method relative to ElasticMorph in Section~\ref{sec:elasticmorph_indirect}.
...
\begin{figure}[t]
\centering
\IfFileExists{HER.png}{\includegraphics[width=0.96\textwidth]{HER.png}}{\IfFileExists{../HER.png}{\includegraphics[width=0.96\textwidth]{../HER.png}}{\IfFileExists{../figures/HER.png}{\includegraphics[width=0.96\textwidth]{../figures/HER.png}}{\includegraphics[width=0.96\textwidth]{figures/fig1_framework_figma.png}}}}
...
\noindent\textbf{Sensitivity of $(\beta,\gamma)$ and $\epsilon$.} A coarse two-axis validation sweep over $\beta$ and $\gamma$ identified a broad optimum around $(\beta,\gamma)=(0.02,20)$, in which validation grouped-Dice remained within one operating-point standard deviation and the non-positive Jacobian ratio remained below $10^{-4}$; this broad-optimum behavior mirrors the weight-sweep finding reported in~\cite{elasticmorph2025} for linear-elastic regularization. The clamping constant $\epsilon$ appears identically in $\mathcal{L}_{\mathrm{volume}}$ (denominator floor) and $\mathcal{L}_{\mathrm{fold}}$ (hinge threshold); values in $[10^{-4},10^{-2}]$ produced visually indistinguishable validation curves and order-preserving rankings between methods, so the choice $\epsilon=10^{-3}$ is not load-bearing for the reported conclusions.
...
\subsection{Indirect Comparison with ElasticMorph}\label{sec:elasticmorph_indirect}
The most closely related concurrent work is the linear-elastic ElasticMorph framework~\cite{elasticmorph2025}, which adopts the same IXI atlas-to-subject protocol (576 T1-weighted volumes; 403/58/115 train/validation/test split; atlas from \citet{kim2021cyclemorph}) and is plugged into the same TransMorph backbone family. Although the two studies were performed on different hardware and therefore cannot be directly cross-tabulated at the per-case level, the matched protocol allows a meaningful indirect quantitative comparison at the relative-improvement level. Table~\ref{tab:em_indirect} summarizes the comparison.
\begin{table}[H]
\caption{Indirect comparison of HypEReg and ElasticMorph on the matched IXI 403/58/115 atlas-to-subject protocol. ElasticMorph numbers are taken from the published ranges reported in \cite{elasticmorph2025} across four tested CNN/Transformer backbones; HypEReg numbers are computed on the present TransMorph backbone (Table~\ref{tab2}).\label{tab:em_indirect}}
\centering
\scriptsize
\setlength{\tabcolsep}{4pt}
\begin{tabularx}{\textwidth}{lXX}
\toprule
\textbf{Reported quantity} & \textbf{ElasticMorph~\cite{elasticmorph2025} (linear-elastic, IXI)} & \textbf{HypEReg-TransMorph (this work, IXI)} \\
\midrule
Regularizer family & Second-order linear-elastic Navier--Cauchy residual (curvature + divergence); small-strain assumption & Non-linear hyperelastic subset: clamped-rational volume term + explicit anti-folding hinge; valid at arbitrary strain magnitude \\
Folding-control guarantee & Asymptotic upper bound on $\#\{\det J_\phi\le 0\}$ under small-strain assumption (Theorem in \cite{elasticmorph2025}) & Per-voxel quadratic supervision on every voxel with $\det J_\phi\le\epsilon$ (no asymptotic regime required) \\
Dice gain over baseline (relative) & $+0.56$\% to $+4.92$\% across four backbones & $+0.13$\% over TransMorph (0.7527 $\to$ 0.7537) \\
Folding-voxel reduction (relative) & $24$\% to $42$\% reduction in \%negJ across four backbones & $99.9$\% reduction in non-positive Jacobian ratio (1.502\% $\to$ 0.0015\%, i.e.\ a $\sim$1{,}000$\times$ decrease) \\
Inference cost & Unchanged & Unchanged (Table~\ref{tab3}) \\
Training overhead & $\le 8$\% time, $\le 15$\% memory & Comparable in scale (added cost is the per-voxel $\det J_\phi$ evaluation; no additional learnable parameters) \\
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Both methods leave inference cost unchanged because the regularization is purely loss-side. The two regularizers occupy complementary positions on the accuracy/regularity Pareto front: ElasticMorph's headline contribution is a Dice gain of up to nearly five points, while HypEReg's headline contribution is a near-elimination of negative-Jacobian voxels (three orders of magnitude reduction). The pattern is mathematically consistent with the underlying penalties: a small-strain Navier--Cauchy residual provides only an asymptotic folding bound, whereas a non-linear hinge on $\det J_\phi$ supplies direct per-voxel folding supervision.}
\end{table}

\subsection{Cross-Cohort Validation on OASIS}\label{sec:oasis_xcohort}
HypEReg-TransMorph is retrained on the OASIS atlas-to-subject release packaged with this repository using the same HypEReg weights as the primary IXI experiment ($\alpha{=}0$, $\beta{=}0.02$, $\gamma{=}20$, $\epsilon{=}10^{-3}$). Registered test pairs (19 pairs from \texttt{OASIS/data/pairs\_val.csv}) are scored with \texttt{OASIS/eval\_oasis.py} and the IXI-compatible metric implementations. Table~\ref{tab:oasis_main} reports mean$\pm$std over all 19 test pairs; paired Wilcoxon signed-rank tests with Benjamini--Hochberg FDR correction against \texttt{transmorph\_her\_oasis} are summarized in Supplementary Table~S5.

On OASIS, HER-TransMorph achieves a non-positive Jacobian ratio of $1.0\times10^{-5}\pm3.3\times10^{-6}$, compared with $0.0081\pm0.0014$ for VoxelMorph-1, CycleMorph, and the published TransMorph (dsc0.857) checkpoint---a reduction exceeding $800\times$ ($q<10^{-4}$, all comparisons BH-FDR). SDlogJ is reduced to $0.2786\pm0.0106$ relative to $0.421$--$0.435$ for these baselines ($q<10^{-4}$), while Dice (0.797) is significantly higher than VoxelMorph-1, CycleMorph, and MIDIR ($q<10^{-5}$). The published semi-supervised checkpoint (dsc0.857) reaches higher Dice (0.861) and lower HD95 (1.60\,mm) because it was trained with direct DSC supervision on OASIS; HER-TransMorph trades a fraction of overlap for a $\sim$800$\times$ reduction in folding---the same qualitative trade-off observed on IXI. MIDIR achieves zero non-positive Jacobian ratio via B-spline parameterization and marginally lower SDlogJ (0.2551) than HER-TransMorph, reproducing the pattern seen on IXI.

\begin{table}[H]
\caption{OASIS cross-cohort evaluation (mean$\pm$std over 19 test pairs, $n{=}19$; paired Wilcoxon vs HER-TransMorph in Supplementary Table~S5).\label{tab:oasis_main}}
\centering
\scriptsize
\setlength{\tabcolsep}{3pt}
\begin{tabularx}{\textwidth}{lccccc}
...
\midrule
HER-TransMorph (OASIS-retrained) & \textbf{0.7966 \(\pm\) 0.0224} & \underline{$1.0\times10^{-5}$ \(\pm\) $3.3\times10^{-6}$} & \underline{0.2786 \(\pm\) 0.0106} & \underline{2.2943 \(\pm\) 0.4891} & 0.6801 \(\pm\) 0.0891 \\
TransMorph (dsc0.857, published) & \textbf{0.8610 \(\pm\) 0.0138} & 0.008069 \(\pm\) 0.001386 & 0.4211 \(\pm\) 0.0185 & \textbf{1.5980 \(\pm\) 0.3096} & \textbf{0.4829 \(\pm\) 0.0497} \\
TransMorph (unsup, IXI zero-shot) & --- & --- & --- & --- & --- \\
VoxelMorph-1 & 0.7159 \(\pm\) 0.0477 & 0.008075 \(\pm\) 0.001318 & 0.4291 \(\pm\) 0.0177 & 3.1806 \(\pm\) 0.6656 & 0.9632 \(\pm\) 0.1684 \\
MIDIR & 0.7254 \(\pm\) 0.0291 & \textbf{0.000000 \(\pm\) 0.000000} & \textbf{0.2551 \(\pm\) 0.0123} & 2.8926 \(\pm\) 0.4944 & \underline{0.9306 \(\pm\) 0.1079} \\
CycleMorph & 0.7243 \(\pm\) 0.0388 & 0.008190 \(\pm\) 0.001134 & 0.4345 \(\pm\) 0.0165 & 3.0759 \(\pm\) 0.5840 & 0.9485 \(\pm\) 0.1380 \\
SyN (ANTs) & --- & --- & --- & --- & --- \\
Affine (MI) & --- & --- & --- & --- & --- \\
\bottomrule
\end{tabularx}
\parbox{\textwidth}{\footnotesize Values: mean\(\pm\)std over 19 OASIS test pairs. Bold = best per metric; underline = second-best. ``unsup IXI zero-shot'' uses IXI-trained TransMorph checkpoint without OASIS retraining; SyN/Affine/unsup rows pending completion of background evaluation. HER-TransMorph uses OASIS-retrained checkpoint (\texttt{dsc0.7912.pth.tar}).}
\end{table}

The indirect comparison emphasizes that HypEReg is not a redundant re-implementation of ElasticMorph: the two methods optimize for different regions of the trade-off and rely on different mathematical mechanisms (small-strain elastic residual versus non-linear Jacobian-determinant control). A direct head-to-head retraining of both regularizers on bit-identical hardware, data, and optimizer schedules, with repeated seeds, remains important future work beyond the present indirect IXI comparison and the dedicated OASIS retraining protocol.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/fig2_qualitative.png}
...
Our work sits in a focused research line that injects physically motivated regularization into learned registration. The selective Jacobian-determinant penalty of \citet{mok2020fast} contributes only the hinge component of $\mathcal{L}_{\mathrm{fold}}$, the BMR volume-change plus curvature regularizer of \citet{hering2021lung} is restricted to CNN lung-CT registration, the conformal-invariant hyperelastic energy of \citet{zou2023conformal} is also CNN- and lung-CT--specific, and the recent \emph{ElasticMorph} framework~\cite{elasticmorph2025} relies on a second-order linear-elastic Navier--Cauchy residual whose folding-control guarantee holds only asymptotically under small-strain assumptions. HypEReg differs along three concrete axes that, jointly, define a new operating point in this design space:
\begin{itemize}\setlength\itemsep{2pt}
\item \emph{Loss locus.} HypEReg acts directly and non-linearly on $\det J_\phi$, the geometric quantity used downstream for tensor-based morphometry, rather than on linear-elastic divergence/curl operators or on first-order strain tensors; it is therefore mathematically valid at arbitrary strain magnitudes and is not approximating any limit regime.
\item \emph{Folding control.} The explicit anti-folding hinge supplies a per-voxel quadratic penalty wherever $\det J_\phi\le\epsilon$, which empirically delivers a $\sim$1{,}000$\times$ reduction in non-positive Jacobian ratio (Section~\ref{sec:elasticmorph_indirect}); the linear-elastic surrogate of~\cite{elasticmorph2025} reports a $24$--$42\%$ reduction in $\%$negJ across four backbones, indicating the two mechanisms occupy complementary regions of the accuracy/regularity Pareto front.
\item \emph{Backbone and protocol.} HypEReg is evaluated on a Transformer dense-flow backbone with paired Wilcoxon signed-rank tests and Benjamini--Hochberg FDR control on subject-level metrics; the indirect comparison in Section~\ref{sec:elasticmorph_indirect} aligns directly with the matched IXI 403/58/115 protocol used by ElasticMorph.
\end{itemize}
Within this positioning, a controlled head-to-head retraining of both regularizers on bit-identical hardware/data/optimizer schedules, with repeated seeds, is the natural experimental complement to the present indirect IXI comparison; we treat it as future work beyond the OASIS cross-cohort validation reported here, but emphasize that it would resolve the relative \emph{magnitude} of the two effects rather than the qualitative finding that linear-elastic and hyperelastic-Jacobian regularization populate distinct regions of the accuracy/regularity trade-off.
