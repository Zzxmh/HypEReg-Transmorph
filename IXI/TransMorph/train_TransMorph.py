...
def main():
    ...
    train_dir = os.path.join(ixi_root, "Train")
    val_dir = os.path.join(ixi_root, "Val")
    batch_size = 2
    weights = [1.0, 1.0]  # NCC : Grad (same as PSC IXI default)
    save_dir = "TransMorph_IXI_ncc_{}_grad_{}/".format(weights[0], weights[1])
    ...
    print(
        f"IXI TransMorph: train={len(train_paths)} val={len(val_paths)} atlas={atlas_dir}"
    )
