...
file_dir = 'Results/'
file_name = ['CoTr_ncc_1_diffusion_1', 'PVT_ncc_1_diffusion_1', 'ViTVNet_ncc_1_diffusion_1', 'nnFormer_ncc_1_diffusion_1', 'TransMorphDiff', 'TransMorphBspline_ncc_1_diffusion_1', 'TransMorphBayes_ncc_1_diffusion_1', 'TransMorph_ncc_1_diffusion_1', 'TransMorph_HER_IXI']
...
# 与 IXI/Results 中各模型 Test 推理 CSV 一致：115 个病例/列
N_SUBJ = 115
...
for exp_name in file_name:
    print(exp_name)
    exp_data = np.zeros((len(outstruct), N_SUBJ))
    stct_i = 0
...
her = plt.boxplot(all_data[8].T, labels=outstruct, positions=np.array(range(len(outstruct)))*spacing_factor+sep/2+4*sep, widths=0.6, showmeans=showmeans, flierprops=flierprops, meanprops=meanprops, patch_artist=True)
...
for tick in ax.get_yticklabels():
    tick.set_fontname("Cambria")
plt.xlim(-5, len(outstruct)*spacing_factor-3.0)
plt.ylim(-0.05, 1.05)
plt.tight_layout()
plt.xticks(rotation=90)
plt.gcf().subplots_adjust(bottom=0.4)
out_png = os.path.join(file_dir, 'boxplot_baseline_incl_her.png')
