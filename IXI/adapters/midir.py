...
from ._helpers import add_sys_path, cfg_with_size, ixi_root, load_state_dict_any
from ._helpers import purge_module_prefix
...
def forward(model, x, y):
    ...
    flow = F.interpolate(
        disp, size=x.shape[2:], mode="trilinear", align_corners=True
    )
    return x_def, flow
